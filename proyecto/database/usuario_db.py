...
def login(usuario: User) -> User:
    sql = " SELECT name, email, password_hash FROM users WHERE email = '{}' ".format(usuario.email) #la variable del modelo User
    row = _fetch_one(sql,None)
    print('---------- DAATOS DE CAPA DATO LOGIN')
    print(row)
    if row !=None:
        usuario = User(row[0],row[1], (User.check_password(row[2],(usuario.password_hash))))
        print(usuario)
